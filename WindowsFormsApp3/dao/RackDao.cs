...
public class RackDao
{
    ...
    public void AddRack(Rack rack)
    {
        ...
        connection.Open();
        var sql = "INSERT INTO Racks (location, capacity) VALUES (@Location, @Capacity)";
    ...
    public Rack GetById(int rackId)
    {
        using var connection = new MySqlConnection(connectionString);
        connection.Open();
        var sql = "SELECT * FROM Racks WHERE rack_id = @RackId";
        using var command = new MySqlCommand(sql, connection);
    ...
    public void UpdateRack(Rack rack)
    {
        using var connection = new MySqlConnection(connectionString);
        connection.Open();
        var sql = "UPDATE Racks SET location = @Location, capacity = @Capacity WHERE rack_id = @RackId";
        using var command = new MySqlCommand(sql, connection);
        command.Parameters.AddWithValue("@Location", rack.Location);
        command.Parameters.AddWithValue("@Capacity", rack.Capacity);
    ...
    public void DeleteRack(int rackId)
    {
        using var connection = new MySqlConnection(connectionString);
        connection.Open();
        var sql = "DELETE FROM Racks WHERE rack_id = @RackId";
        using var command = new MySqlCommand(sql, connection);
        command.Parameters.AddWithValue("@RackId", rackId);
        command.ExecuteNonQuery();
    }
    ...
    public void PlaceProductOnRack(int productId, int rackId)
    {
        using var connection = new MySqlConnection(connectionString);
        connection.Open();
        var sql = "INSERT INTO RackProducts (product_id, rack_id) VALUES (@ProductId, @RackId)";
        using var command = new MySqlCommand(sql, connection);
