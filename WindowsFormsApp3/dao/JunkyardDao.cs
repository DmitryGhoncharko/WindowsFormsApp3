...
public class JunkyardDao
{
    ...
    public void AddToJunkyard(int productId, int quantity)
    {
        ...
        connection.Open();
        var sql = "INSERT INTO Junkyard (product_id, quantity) VALUES (@ProductId, @Quantity)";
    ...
    {
        var items = new List<Junkyard>();
        using var connection = new MySqlConnection(connectionString);
        connection.Open();
        var sql = "SELECT * FROM Junkyard";
        using var command = new MySqlCommand(sql, connection);
    ...
    public void UpdateJunkyardItem(int junkId, int quantity)
    {
        using var connection = new MySqlConnection(connectionString);
        connection.Open();
        var sql = "UPDATE Junkyard SET quantity = @Quantity WHERE junk_id = @JunkId";
        using var command = new MySqlCommand(sql, connection);
        command.Parameters.AddWithValue("@Quantity", quantity);
    ...
    public void DeleteFromJunkyard(int junkId)
    {
        using var connection = new MySqlConnection(connectionString);
        connection.Open();
        var sql = "DELETE FROM Junkyard WHERE junk_id = @JunkId";
        using var command = new MySqlCommand(sql, connection);
        command.Parameters.AddWithValue("@JunkId", junkId);
        command.ExecuteNonQuery();
    }
